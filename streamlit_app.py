...
SYSTEM_PROMPT = f"""Você é o CineBot, agente especialista em cinema com acesso a um banco SQLite de 12.404 filmes reais do IMDb.

SCHEMA DO BANCO:
{get_schema()}

Você opera em loop ReAct. Para responder perguntas sobre filmes, você DEVE consultar o banco usando SQL.

FORMATO OBRIGATÓRIO quando precisar de dados:
THOUGHT: [seu raciocínio]
SQL: [query SQLite válida]

Quando tiver a resposta final, responda normalmente em português brasileiro sem usar THOUGHT/SQL.
NUNCA invente dados. SEMPRE consulte o banco para fatos concretos.
Use LIKE para buscas de texto (ex: genre LIKE '%Drama%').
Limite resultados com LIMIT 10 por padrão."""

def call_llama(bedrock, prompt: str) -> str:
    body = json.dumps({
        "prompt": prompt,
        "max_gen_len": 1500,
        "temperature": 0.3,
    })
    result = json.loads(bedrock.invoke_model(modelId=MODEL_ID, body=body)['body'].read())
...
def run_agent(user_message: str, history: list, key_id: str, secret: str, session_token: str, region: str) -> str:
    ...
    for h in history[-6:]:  # last 3 turns
        role = "Usuário" if h['role'] == 'user' else "CineBot"
...
{hist_text}Usuário: {user_message}
{context}
<|start_header_id|>assistant<|end_header_id|>"""

        response = call_llama(bedrock, prompt)

        # Check if model wants to run SQL
        sql_match = re.search(r'SQL:\s*(.+?)(?:\n|$)', response, re.IGNORECASE | re.DOTALL)
        if sql_match:
            sql = sql_match.group(1).strip().rstrip(';') + ';'
            # Clean up SQL if wrapped in backticks
            sql = re.sub(r'```sql?|```', '', sql).strip()
            db_result = query_db(sql)
            context += f"\nRESULTADO SQL: {db_result}\n"
            continue

        # No SQL = final answer
        # Clean up any leftover THOUGHT: prefixes
        answer = re.sub(r'^THOUGHT:.*?\n', '', response, flags=re.MULTILINE).strip()
        return answer if answer else "Não consegui processar sua pergunta."

    return "Não consegui encontrar a resposta. Tente reformular."
...
with st.sidebar:
    ...
    if not st.session_state.configured:
        ...
        <div class="info-row"><span class="info-key">Modelo</span><span class="info-val">LLaMA 4 Scout</span></div>
